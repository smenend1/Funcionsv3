...
function executar() {
    ...
    try {
        if (mode === 'punts') {
            ...
            puntsTaula = [-2, -1, 0, 1, 2];
        }

        // Generar Taula (abans de la gràfica per seguretat)
        puntsTaula = [...new Set(puntsTaula)].sort((a,b) => a-b);
        let taulaHTML = `<table style="width:100%; margin-top:10px; border-collapse:collapse; font-size:0.9rem;" border="1">
            <tr style="background:#eee"><th>x</th><th>f(x) (Frac)</th><th>f(x) (Dec)</th></tr>`;
        puntsTaula.forEach(px => {
            const valY = f(px);
            taulaHTML += `<tr><td>${px.toFixed(1)}</td><td>${decimalAFraccio(valY)}</td><td>${valY.toFixed(2)}</td></tr>`;
        });
        taulaHTML += `</table>`;
